...
import {
  ...
  removeBooksRead, removeBooksToRead,
  addBooksRead
} from '../API/books.api';
  ...
  const getList = async v => {
    try {
      ...
      if (v === 1) {
        ...
        // );
        const response = await retrieveBooksToRead({ user_id: user.id });
        setList(1);
        ...
        // );
        const response = await retrieveBooksRead({ user_id: user.id });
        setList(2);
        setMyListOfBooks(response.data.books_read);
      }
